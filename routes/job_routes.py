...
async def create_job_page(request: Request, db: Session = Depends(get_db)):
    user = get_current_user(request, db)
    if not user or user.role != "recruiter":
        return RedirectResponse("/", status_code=303)
    return csrf_response(request, templates, "job_create.html", {"user": user})
...
async def create_job(
    ...
    if not validate_csrf_token(csrf_token):
        return RedirectResponse("/jobs/create?error=csrf", status_code=303)
 
    user = get_current_user(request, db)
    if not user or user.role != "recruiter":
        return RedirectResponse("/", status_code=303)
 
    job = JobPosting(title=title, description=description,
    ...
    log_action(db, actor_id=user.id, action="JOB_CREATED",
               target=f"job:{job.id}",
               detail=(f"Recruiter {user.email} created job '{title}' "
...
async def job_detail(job_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if not job:
        return RedirectResponse("/jobs", status_code=303)
    return csrf_response(request, templates, "job_detail.html",
...
async def apply_job(
    ...
    if not validate_csrf_token(csrf_token):
        return RedirectResponse(f"/jobs/{job_id}?error=csrf", status_code=303)
    ...
    application = Application(job_id=job_id, user_id=user.id, cover_note=cover_note)
    db.add(application)
    db.commit()
    db.refresh(application)
 
    log_action(db, actor_id=user.id, action="JOB_APPLIED",
               target=f"job:{job_id}",
               detail=(f"User {user.email} applied to job_id={job_id} "
...
async def my_applications(request: Request, db: Session = Depends(get_db)):
    ...
    return csrf_response(request, templates, "applications.html",
