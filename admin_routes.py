...
async def admin_suspend(
    ...
    log_action(
        ...
        detail   = (
            f"Admin {admin.email} {'unsuspended' if previous_state else 'suspended'} "
...
async def admin_delete(
    request: Request,
    user_id: int,
    otp:     str = Form(...),          # TOTP code from virtual keyboard
    db:      Session = Depends(get_db),
    csrf_protect: CsrfProtect = Depends(),
):
    await csrf_protect.validate_csrf(request)
    ...
    # ── HIGH-RISK ACTION: verify admin's TOTP before deleting ──
    # The admin must enter their current authenticator code.
    # This is the second factor for a destructive irreversible action.
    if not verify_totp(admin.otp_secret, otp):
        log_action(
            db,
            actor_id = admin.id,
            action   = "USER_DELETE_FAIL_OTP",
            target   = f"user:{user_id}",
            detail   = (
                f"Admin {admin.email} failed TOTP check "
    ...
    # ── AUDIT: log BEFORE delete (target.id gone after db.delete) ──
    log_action(
        db,
        actor_id = admin.id,
        action   = "USER_DELETE",
        target   = f"user:{target.id}",
        detail   = (
            f"Admin {admin.email} permanently deleted user "
...
async def audit_log_viewer(
    ...
    # ── Run full chain + signature verification ───────────────
    verification = verify_chain(db)

    # ── Fetch all rows newest-first for display ───────────────
    # We need per-row validity flags for the table, so re-walk
    # rows and annotate each with its verification result.
    ...
    # Build a set of bad row IDs from the ascending verification pass
    # for O(1) lookup when rendering the table.
    # We do a second ascending pass to get per-row flags.
    ...
    for r in asc_rows:
        (rid, actor_id, action, target, detail,
         prev_hash, stored_hash, row_signature) = r
        ...
        if row_signature:
            ...
            row_sig_valid[rid] = None  # no signature — not a failure

    # Annotate rows for the template
    class LogRow:
        pass
    ...
    for r in raw_rows:
        (rid, actor_id, action, target, detail,
         prev_hash, row_hash, row_signature, logged_at) = r
        obj                = LogRow()
        obj.id             = rid
        ...
        obj.sig_valid      = row_sig_valid.get(rid)  # None = unsigned
        annotated.append(obj)

    return csrf_response(
        request, templates, "audit_logs.html",
        {
            "user":           admin,
            "logs":           annotated,
